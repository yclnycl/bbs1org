...
define('LOCK_FILE', __DIR__ . '/data/install.lock');
define('SETTING_CACHE_FILE', __DIR__ . '/data/cache_settings.php');

if (file_exists(LOCK_FILE)) die('已安装，请删除 ' . LOCK_FILE . ' 后重试');
...
$db = db();
$db->exec("
CREATE TABLE IF NOT EXISTS groups(id INTEGER PRIMARY KEY,name TEXT NOT NULL UNIQUE,is_admin INTEGER NOT NULL DEFAULT 0,is_banned INTEGER NOT NULL DEFAULT 0,is_muted INTEGER NOT NULL DEFAULT 0);
...
file_put_contents(__DIR__ . '/data/cache_forums.php', "<?php\nreturn " . var_export($forums, true) . ";\n", LOCK_EX);
$groups = $db->query("SELECT id,name,is_admin,is_banned,is_muted FROM groups ORDER BY id")->fetchAll();
file_put_contents(__DIR__ . '/data/cache_groups.php', "<?php\nreturn " . var_export($groups, true) . ";\n", LOCK_EX);
file_put_contents(__DIR__ . '/data/cache_stats.php', "<?php\nreturn " . var_export([
